...
/**
 * Interactive merchant search hero — V1 product-first design.
 *
 * Reference: design_handoff_homepage_v1/v1-product.jsx. A single big
 * search input is the primary CTA; typing reveals a rich dropdown with
 * best-card / est-THB / chevron cues. Selector is demoted to a text
 * fallback beneath. Popular-search chips seed common queries.
 */

const POPULAR = ['Starbucks', 'Lazada', 'Agoda', 'Gourmet', 'Grab'] as const;
const DEBOUNCE_MS = 200;

export function HeroSearch({ initialCount }: { initialCount?: number }) {
  ...
  // ⌘K / Ctrl+K focus shortcut — matches the kbd hint in the input.
  React.useEffect(() => {
  ...
  return (
    <section
      className="relative overflow-hidden px-4 pb-8 pt-20 md:px-6 md:pb-12 md:pt-28"
      aria-labelledby="hero-heading"
    >
      <div className="mx-auto max-w-[880px] text-center">
        <span className="mb-7 inline-flex items-center gap-2 rounded-full border border-loftly-divider bg-loftly-surface px-3.5 py-1.5 text-body-sm text-loftly-ink-muted">
          <span
            className="inline-block h-1.5 w-1.5 rounded-full bg-loftly-success"
            aria-hidden
          />
          {t('statusPill', { count: initialCount ?? 160 })}
        </span>
        ...
        <p
          className="mx-auto mb-9 max-w-xl text-loftly-ink-muted"
          style={{ fontSize: 19, lineHeight: 1.5, textWrap: 'pretty' }}
        >
          {t('subheading')}
        </p>
        ...
        <form onSubmit={handleSubmit} role="search" className="relative mx-auto max-w-[640px]">
          <div
            className={cn(
              'flex items-center gap-3.5 rounded-[14px] bg-loftly-surface px-5 py-4 transition-all duration-200 md:px-6 md:py-5',
              focused
          ...
          >
            ...
            <input
              ...
              autoComplete="off"
              spellCheck={false}
              className="flex-1 bg-transparent text-[18px] text-loftly-ink placeholder:text-loftly-ink-subtle focus:outline-none"
            />
            {query ? (
              <button
                type="button"
            ...
            >
              {results.map((m, i) => (
                <Link
                  ...
                  aria-selected={i === 0}
                  className={cn(
                    'grid grid-cols-[auto_1fr_auto_auto] items-center gap-4 px-5 py-3.5 transition-colors hover:bg-loftly-surface-muted',
                    i > 0 && 'border-t border-loftly-divider',
                  )}
                >
                  <span className="flex h-10 w-10 items-center justify-center rounded-[10px] bg-loftly-surface-muted text-[14px] font-semibold text-loftly-ink-muted">
              ...
              <div className="flex items-center justify-between bg-loftly-surface-muted px-5 py-3 text-caption text-loftly-ink-subtle">
        ...
        <p className="mt-12 text-body-sm text-loftly-ink-subtle">
          {t('altCtaLead')}{' '}
          <Link
            href="/selector"
