...
export default async function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen bg-loftly-teal-soft/40">
      <aside className="hidden w-56 shrink-0 border-r border-loftly-divider bg-white md:block">
        ...
        <nav className="flex flex-col gap-1 px-3 pb-6 text-sm">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-3 py-2 text-loftly-ink hover:bg-loftly-divider/50"
            >
              {item.label}
            </Link>
          ))}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center justify-between border-b border-loftly-divider bg-white px-6 py-3">
          <div className="flex items-center gap-3 md:hidden">
            <Link href="/admin/cards" className="font-semibold">
              Loftly Admin
            </Link>
          </div>
          <div className="ml-auto flex items-center gap-3 text-sm text-loftly-ink-muted">
            <span>Signed in</span>
            <form action="/api/auth/logout" method="post">
              <button
                type="submit"
                className="rounded-md border border-loftly-divider px-3 py-1.5 text-sm hover:bg-loftly-teal-soft/40"
              >
                Logout
              </button>
            </form>
